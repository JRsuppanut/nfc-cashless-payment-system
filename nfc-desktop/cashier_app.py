...
class NFCBridge(QObject):
    """ส่งข้อมูลจาก NFC Worker กลับมายัง UI Thread"""
...
class CashierApp(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        # ข้อมูลรายการเติมเงินที่กำลังรอยืนยัน
        ...
        self.nfc_bridge.tag_detected.connect(
            self._handle_card_detected
        )
        self.nfc_bridge.tag_removed.connect(
        ...
        # ใช้ NFC Worker เดิมของโปรเจกต์
    ...
    def _init_ui(self) -> None:
        ...
        self.page_stack.addWidget(
            self._create_issue_page()
        )
        self.page_stack.addWidget(
            self._create_top_up_page()
        )
        self.page_stack.addWidget(
            self._create_check_page()
        )
        self.page_stack.addWidget(
            self._create_return_page()
        )

        body_layout.addWidget(self.page_stack, 56)
        body_layout.addWidget(
            self._create_ledger_panel(),
            44,
        )
    ...
    def _create_header(self) -> QFrame:
        ...
        self.reader_status_label.setObjectName(
            "readerStatus"
        )
    ...
    def _create_navigation(self) -> QHBoxLayout:
        ...
        for index, text in enumerate(navigation_items):
            ...
            button.clicked.connect(
                lambda checked=False, mode=index:
    ...
    def _create_issue_page(self) -> QWidget:
        page, layout = self._create_page(
            "Set Up a New Card",
            "Place a new or returned NFC card on "
            "the reader, then confirm.",
        )

        card_panel, card_layout = self._create_panel(
            soft=True
        )

        self.issue_status_title = QLabel(
            "Ready for a New Card"
        )
        self.issue_status_title.setObjectName(
            "largeStatus"
        )

        self.issue_status_message = QLabel(
            "Place the card on the NFC reader."
        )
        self.issue_status_message.setObjectName(
            "mutedText"
        )
        ...
        card_layout.addWidget(
            self.issue_status_title
        )
        card_layout.addWidget(
            self.issue_status_message
        )
        ...
        data_rows = [
            ...
            (
                "On-card Authentication Code",
                "Generated using HMAC-SHA256",
            ),
            (
                "Database starting balance",
                "฿0.00",
            ),
        ]
        ...
        self.btn_provision = QPushButton(
            "Confirm and Set Up Card"
        )
        self.btn_provision.setObjectName(
            "primaryButton"
        )
        self.btn_provision.setEnabled(False)
        self.btn_provision.clicked.connect(
            self._handle_provision_action
        )
    ...
    def _create_top_up_page(self) -> QWidget:
        page, layout = self._create_page(
            "Add Money to Card",
            "Enter an amount or choose a common amount. "
            "Then tap the card and confirm the transaction.",
        )

        amount_panel, amount_layout = (
            self._create_panel()
        )
        ...
        self.amount_input.setObjectName(
            "amountInput"
        )
        ...
        scan_panel, scan_layout = (
            self._create_panel(soft=True)
        )

        self.top_up_status_title = QLabel(
            "Ready to Scan"
        )
        self.top_up_status_title.setObjectName(
            "largeStatus"
        )

        self.top_up_status_message = QLabel(
            "Place the card on the NFC reader."
        )
        self.top_up_status_message.setObjectName(
            "mutedText"
        )
        self.top_up_status_message.setWordWrap(True)

        self.top_up_identity = QLabel(
            "UID: —     Token: Not detected"
        )
        self.top_up_identity.setObjectName(
            "mutedText"
        )
        self.top_up_identity.setWordWrap(True)

        scan_layout.addWidget(
            self.top_up_status_title
        )
        scan_layout.addWidget(
            self.top_up_status_message
        )
        scan_layout.addWidget(
        ...
        self.btn_confirm_top_up = QPushButton(
            "Confirm Add Money"
        )
        self.btn_confirm_top_up.setObjectName(
            "primaryButton"
        )
        self.btn_confirm_top_up.setEnabled(False)
        self.btn_confirm_top_up.clicked.connect(
            self._confirm_top_up
        )

        layout.addWidget(
            self.btn_confirm_top_up
        )
        layout.addStretch()

        # หากแก้จำนวนเงินหลังอ่านบัตร
        # จะยกเลิกรายการที่กำลังรอยืนยัน
        self.amount_input.textChanged.connect(
            self._handle_amount_changed
        )
    ...
    def _create_check_page(self) -> QWidget:
        page, layout = self._create_page(
            "Check Card Balance",
            "Tap a card to verify it and display "
            "the current balance.",
        )

        balance_panel, balance_layout = (
            self._create_panel(soft=True)
        )
        ...
        balance_title.setObjectName(
            "sectionTitle"
        )
        balance_title.setAlignment(
            Qt.AlignmentFlag.AlignCenter
        )

        self.check_balance_value = QLabel("—")
        self.check_balance_value.setObjectName(
            "balanceValue"
        )
        self.check_balance_value.setAlignment(
            Qt.AlignmentFlag.AlignCenter
        )

        balance_layout.addWidget(balance_title)
        balance_layout.addWidget(
            self.check_balance_value
        )

        layout.addWidget(balance_panel)

        status_panel, status_layout = (
            self._create_panel()
        )
        ...
        self.check_status.setObjectName(
            "largeStatus"
        )
        ...
        self.check_message.setObjectName(
            "mutedText"
        )
        self.check_message.setWordWrap(True)

        self.check_identity = QLabel(
            "UID: —     Token: Not detected"
        )
        self.check_identity.setObjectName(
            "mutedText"
        )
    ...
    def _create_return_page(self) -> QWidget:
        page, layout = self._create_page(
            "Return Card and Refund Balance",
            "Review the remaining balance before "
            "returning the card.",
        )

        values_panel, values_layout = (
            self._create_panel()
        )
        ...
        self.return_card_status = QLabel(
            "Waiting for Card"
        )
        ...
        self.return_warning = QLabel(
            "Tap a card to display the refundable "
            "balance. No change will be made until "
            "you confirm."
        )
        ...
        self.return_identity = QLabel(
            "UID: —     Token: Not detected"
        )
        self.return_identity.setObjectName(
            "mutedText"
        )
        ...
        self.btn_return = QPushButton(
            "Confirm Refund and Return Card"
        )
        self.btn_return.setObjectName(
            "primaryButton"
        )
        self.btn_return.setEnabled(False)
        self.btn_return.clicked.connect(
            self._handle_return_action
        )
    ...
    def _create_ledger_panel(self) -> QFrame:
        ...
        subtitle = QLabel(
            "Latest cashier and stall transactions"
        )
    ...
    def _switch_mode(self, mode: int) -> None:
        ...
        if mode == self.MODE_ISSUE:
            ...
            self._set_top_up_feedback(
                ...
                "The current card has already been read. "
                "Remove it and tap it again after selecting "
                "the amount.",
                None,
            )
    ...
    def _prepare_issue_view(self) -> None:
        self.issue_uid.setText(
            f"UID: {self._active_uid or '—'}"
        )
        ...
        if not self._is_card_authenticated:
            if (
                self._last_card_status
                == "FOREIGN_OR_UNINITIALIZED_CARD"
            ):
                self.issue_status_title.setText(
                    "New Card Detected"
                )
                self.issue_status_message.setText(
                    "Confirm to write a secure "
                    "food court token."
                )
                ...
                self.issue_status_message.setText(
                    f"Card error: "
                    f"{self._last_card_status}"
                )
                self.btn_provision.setEnabled(False)

            return

        wallet = database.get_wallet_by_token(
            self._active_token_uuid
        )

        if (
            wallet is None
            or wallet["status"] == "AVAILABLE"
        ):
            self.issue_status_title.setText(
                "Returned Card Ready"
            )
            self.issue_status_message.setText(
                "Confirm to assign a new token "
                "to this reusable card."
            )
            self.btn_provision.setEnabled(True)
            ...
            self.issue_status_message.setText(
                "Return the card and refund its "
                "balance before setting it up again."
            )
            self.btn_provision.setEnabled(False)
    ...
    def _handle_provision_action(self) -> None:
        ...
        confirm = QMessageBox.question(
            self,
            "Confirm Card Setup",
            "Set up this card with a new food court "
            "token and a starting balance of ฿0.00?",
        ...
        success, result = (
            self.nfc_worker.provision_active_card()
        )

        if success:
            token_uuid = result

            database.register_provisioned_card(
                token_uuid,
                self._active_uid,
            )
            ...
            self.issue_status_message.setText(
                "The card is active with a "
                "starting balance of ฿0.00."
            )
            self.btn_provision.setEnabled(False)
    ...
    def _prepare_top_up_confirmation(self) -> None:
        ...
        if (
            not self._is_card_authenticated
            or not self._active_token_uuid
        ):
            self._set_top_up_feedback(
                "Card Rejected",
                f"The card could not be verified "
                f"({self._last_card_status}).",
                False,
            )
            return
        ...
        try:
            ...
            self._set_top_up_feedback(
                ...
                "Then remove and tap the card again.",
                False,
            )
            return
        ...
        if wallet["status"] != "ACTIVE":
            self._set_top_up_feedback(
                "Card Is Not Active",
                "This card cannot receive money "
        ...
        self.btn_confirm_top_up.setText(
            f"Confirm Add ฿{amount:,.2f}"
        )
    ...
    def _confirm_top_up(self) -> None:
        ...
        if (
            ...
            QMessageBox.warning(
                self,
                "Add Money",
                "The card was removed or changed. "
                "Please tap the card again.",
            )
            return
        ...
        if success:
            self._set_top_up_feedback(
                ...
                f"Remove the card before the next transaction.",
                True,
            )
    ...
    def _handle_amount_changed(
        ...
        self._set_top_up_feedback(
            "Amount Changed",
            "The previous confirmation was cancelled. "
            "Remove the card and tap it again to "
            "confirm the new amount.",
            None,
        )

    def _clear_pending_top_up(
        self,
        ...
        if (
            not keep_message
            and hasattr(self, "top_up_status_title")
            and not self._active_uid
        ):
            self._set_top_up_feedback(
                "Ready to Scan",
                "Place the card on the NFC reader.",
    ...
    def _prepare_return_view(self) -> None:
        ...
        if (
            ...
            self.return_card_status.setText(
                "Card Rejected"
            )
        ...
        if wallet is None:
            ...
            self.return_warning.setText(
                "No wallet record exists for this card."
            )
            return
        ...
        if status == "ACTIVE":
            self.return_warning.setText(
                "After confirmation, the balance will "
                "become ฿0.00 and the card will be "
                "available for reuse."
            )
    ...
    def _handle_return_action(self) -> None:
        ...
        confirm = QMessageBox.question(
            ...
            f"Refund ฿{refund_amount:,.2f} and "
            f"make this card available for reuse?",
            QMessageBox.StandardButton.Yes
            | QMessageBox.StandardButton.No,
        )
        ...
        success, message, refunded_amount = (
            database.process_card_return(
                self._active_token_uuid,
                self._active_uid,
            )
        )
    ...
    def _set_top_up_feedback(
        ...
        self.top_up_status_message.setText(
            message
        )
        ...
        self.top_up_status_title.setStyleSheet(
            f"color: {color};"
        )
    ...
    def _friendly_error(code: str) -> str:
        messages = {
            ...
            "CARD_NOT_PRESENT":
                "Keep the card on the reader "
                "and try again.",

            "HARDWARE_NOT_INITIALIZED":
                "The NFC reader is not connected.",
        }
    ...
    def _refresh_ledger(self) -> None:
        records = database.get_recent_transactions(
            20
        )
    ...
    def closeEvent(self, event) -> None:
        if (
            ...
            and self.nfc_worker is not None
        ):
