...
class NFCWorker(threading.Thread):
    ...
    def provision_active_card(self, expected_uid: Optional[str] = None) -> Tuple[bool, str]:
        with self._lock:
            if not self._pn532:
            ...
            if expected_uid and ":".join(f"{b:02X}" for b in raw_uid) != expected_uid:
